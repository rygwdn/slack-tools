  ...
  server.tool(
    'slack_channel_search',
    {
      query: z.string().describe(
        'A search term to find Slack channels. Can be a channel name or partial match.'
      ),
    },
    async ({ query }) => {
      try {
        ...
        if (!allChannels.length) {
          return {
            content: [{ type: 'text', text: 'No channels found in the workspace or you don\'t have access to any channels.' }],
            isError: true,
          };
        }
        ...
        const matchingChannels = allChannels.filter(channel => {
          ...
          return (
            ...
            (channel.purpose?.value && channel.purpose.value.toLowerCase().includes(cleanQuery.toLowerCase()))
          );
        });
        ...
        formattedResults.forEach(channel => {
          ...
          const truncatedTopic = channel.topic.length > 30 
